...
<!--  データがないときは「データを登録しよう！」みたいなイメージを表示させたい  -->
    <v-row class="ma-5">
      <v-col align="center">
        <v-btn rounded>TODAY</v-btn>
      </v-col>
    </v-row>
    <v-row justify="center">
      <v-col v-for="(item, i) in items" :key="i" cols="9">
        <v-card :color="item.color" dark class="hover-card">
          <div class="d-flex flex-no-wrap">
            <v-col cols="4" class="pa-3">
              <v-avatar size="auto" tile>
                <v-img :src="item.src" :style="{ 'max-width': '80%', 'max-height': '80%' }"></v-img>
              </v-avatar>
            </v-col>
            ...
            <v-col cols="4" class="pa-3">
              ...
              <v-card-text class="word-wrap normal">{{ item.text }}</v-card-text>
              <v-card-actions>
